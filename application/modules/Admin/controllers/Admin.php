...
function view(){
			$data['record'] = $this->M_admin->tampilkan_data();
			$this->load->view('lihat_data', $data);
}
	function lihat_data_detil($id)
	{
		$data['title'] = "Lihat user";
		$data['record'] = $this->M_admin->get_user($id)->row_array();
  ...
  function verifikasipassword($id){
    $pass = md5($this->input->post('password'));
  $query = $this->db->where('user_id', $id)->get('lab_user');
  $array = $query->row();
    if($array->user_pass == $pass){
      $data['hasil'] = 'benar';
