...
class struktur extends CI_Controller
{
	...
	function index()
	{
		$data['title'] = "Manajemen Artikel Struktur";
		$data['record'] = $this->model_struktur->tampilkan_data();
		$this->template->load('template_backend','backend/struktur/lihat_data',$data);
	}

	function lihat_data_detil()
	{
		$data['title'] = "Lihat Artikel Struktur";
		$id = $this->uri->segment(3);
		$data['record'] = $this->model_struktur->get_user($id)->row_array();
		$this->template->load('template_backend','backend/struktur/lihat_data_detil',$data);
	}
	...
	function post()
	{
		if (isset($_POST['submit'])) {
			...
			$data['title'] = 'Tambah Artikel struktur';
			$data['user_profil_nama'] = $this->model_user->tampilkan_data()->result();
			$this->template->load('template_backend','backend/struktur/form_input',$data);
		}
	}
	...
	function edit()
	{
		if (isset($_POST['submit'])) {
			...
			$data['title'] = "Edit Artikel struktur";
			$id = $this->uri->segment(3);

			$data['record'] = $this->model_struktur->get_user($id)->row_array();
			$data['user_profil_nama'] = $this->model_user->tampilkan_data()->result();
			$this->template->load('template_backend','backend/struktur/form_edit',$data);
		}
	}
