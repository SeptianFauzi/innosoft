...
* 
*/
class Artikel_uika extends CI_Controller
{
	
	function __construct()
	{
		parent::__construct();
		$this->load->model('model_artikel_uika');
		$this->load->model('model_user');
	}

	function index()
	{
		$data['title'] = "Manajemen Artikel UIKA";
		$data['record'] = $this->model_artikel_uika->tampilkan_data();
		$this->template->load('template_backend','backend/artikel_uika/lihat_data',$data);
	}

	function lihat_data_detil()
	{
		$data['title'] = "Lihat Artikel UIKA";
		$id = $this->uri->segment(3);
		$data['record'] = $this->model_artikel_uika->get_user($id)->row_array();
		$this->template->load('template_backend','backend/artikel_uika/lihat_data_detil',$data);
	}
	...
	function post()
	{
		if (isset($_POST['submit'])) {
			...
			redirect('artikel_uika');
		} else {
			$data['title'] = 'Tambah Artikel UIKA';
			$data['user_profil_nama'] = $this->model_user->tampilkan_data()->result();
			$this->template->load('template_backend','backend/artikel_uika/form_input',$data);
		}
	}
	...
	function edit()
	{
		if (isset($_POST['submit'])) {
			...
			$data['title'] = "Edit Artikel UIKA";
			$id = $this->uri->segment(3);

			$data['record'] = $this->model_artikel_uika->get_user($id)->row_array();
			$data['user_profil_nama'] = $this->model_user->tampilkan_data()->result();
			$this->template->load('template_backend','backend/artikel_uika/form_edit',$data);
		}
	}
	...
	{
		$id = $this->uri->segment(3);
		$this->model_artikel_uika->hapus($id);
		redirect ('artikel_uika');
	}
}